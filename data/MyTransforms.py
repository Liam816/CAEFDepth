...
class ToTensor(object):
    def __init__(self, test=False, maxDepth=1000.0):
        ...
        # print('maxDepth in class ToTensor:{}'.format(maxDepth))

    def __call__(self, sample):
        image, depth = sample['image'], sample['depth']
        transformation = transforms.ToTensor()

        if self.test:  # 测试集或验证集 test or val
            """
            ...
            # print('transforms.ToTensor() has done.')
            # print('val depth max:{} min:{}'.format(torch.max(depth), torch.min(depth)))

        else:  # 训练集或验证集 train or val
            # Fix for PLI=8.3.0
            image = np.array(image).astype(np.float32) / 255.0
            ...
            zero_mask = depth == 0.0
            image, depth = transformation(image), transformation(depth)
            # NOTE: Guided Decoding 论文使用DepthNorm训练
            depth = torch.clamp(depth, self.maxDepth / 100.0, self.maxDepth)  # [10, 1000]
            depth = self.maxDepth / depth
            depth[:, zero_mask] = 0.0

            # print('train depth max:{} min:{}'.format(torch.max(depth), torch.min(depth)))
