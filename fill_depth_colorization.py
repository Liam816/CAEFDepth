...
def normalize_img(x: np.ndarray):
    min_val = np.min(x)
    max_val = np.max(x)
...
if __name__ == '__main__':
    img_path = '/home/glw/hp/datasets/kitti/0000000005_rgb.png'
    # img_path = '/home/glw/hp/datasets/nyu_data/data/nyu2_train/study_room_0005b_out/99.jpg'
    image = Image.open(img_path)
    image_np = np.array(image)  # .astype(np.float32)

    depth_path = '/home/glw/hp/datasets/kitti/0000000005_depth.png'
    depth = Image.open(depth_path)
    depth_np = np.array(depth)  # .astype(np.float32)

    print('image_np.shape:{}'.format(image_np.shape))
    print('max:{} min:{}'.format(np.max(image_np), np.min(image_np)))
    print('depth_np.shape:{}'.format(depth_np.shape))
    print('max:{} min:{}'.format(np.max(depth_np), np.min(depth_np)))
    exit()
