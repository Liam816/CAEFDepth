...
def arguments_activation_fn(parser: argparse.ArgumentParser):
    ...
    group.add_argument("--model.activation.name", default="relu", type=str,
                       help="Non-linear function name")
    group.add_argument("--model.activation.inplace", action="store_true",
                       help="Use non-linear functions inplace")
    group.add_argument("--model.activation.neg-slope", default=0.1, type=float,
...
for file in os.listdir(act_dir):
    ...
    if (
        ...
        module = importlib.import_module("nets.layers.activation." + model_name)
