...
class MobileViTBlock(BaseModule):
    ...
    def __init__(
        ...
        **kwargs
        ...
        conv_3x3_out = None
        if not no_fusion:  # no_fusion=False 也就是默认需要fusion
    ...
    def unfolding(self, feature_map: Tensor) -> Tuple[Tensor, Dict]:
        ...
        new_h = int(math.ceil(orig_h / self.patch_h) * self.patch_h)  # 用以判断是否能整除
    ...
    def forward_temporal(
        ...
        # convert feature map to patches
        patches, info_dict = self.unfolding(fm)  # patches = [BP, N, C] P为一个patch面积，N为patch数量

        # learn global representations
        for global_layer in self.global_rep:  # 有若干层TransformerEncoder
        ...
        # [B x Patch x Patches x C] --> [B x C x Patches x Patch]
        fm = self.folding(patches=patches, info_dict=info_dict)  # [BP, N, C]->[B, C, H, W]
