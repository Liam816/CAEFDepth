...
def initialize_weights(opts, modules) -> None:
    ...
    logger.debug("LIAM isinstance(modules, nn.Sequential):{}".format(isinstance(modules, nn.Sequential)))
    logger.debug("LIAM isinstance(modules, (nn.Conv2d, nn.Conv3d)):{}".format(isinstance(modules, (nn.Conv2d, nn.Conv3d))))
    logger.debug("LIAM isinstance(modules, norm_layers_tuple):{}".format(isinstance(modules, norm_layers_tuple)))
    logger.debug("LIAM isinstance(modules, (nn.Linear, LinearLayer)):{}".format(isinstance(modules, (nn.Linear, LinearLayer))))
    logger.debug("LIAM isinstance(modules, GroupLinear):{}".format(isinstance(modules, GroupLinear)))
    logger.debug("LIAM modules.__class__.__name__:{}".format(modules.__class__.__name__))
    ...
    if isinstance(modules, nn.Sequential):
        ...
        if isinstance(modules, (nn.Conv2d, nn.Conv3d)):
            ...
            logger.debug("LIAM initialize_weights did nothing!")
