...
from ..layers.adaptive_bins import mViT
...
class MobileViTGUBAdaBins(nn.Module):
    """
    ...
    def __init__(self, opts, bottleneck_num_features=640, decoder_width=1.0, *args, **kwargs) -> None:
        ...
        self.decoder_up5 = Guided_Upsampling_Block(in_features=1,  # up_features[4]
        ...
        self.min_val = 0.1
        self.max_val = 10
        self.adabins_conv = nn.Conv2d(10, 128, kernel_size=1, stride=1, padding=0)
    ...
    def forward(self, x: Any, *args, **kwargs) -> Any:
        ...
        # NOTE: AdaBins
        res = self.adabins_conv(features2[-1])  # res: [b, 128, 240, 320]
        # print("res.shape:", res.shape)
        bin_widths_normed, range_attention_maps = self.adaptive_bins_layer(res)
        # print("bin_widths_normed:{} range_attention_maps:{}".format(bin_widths_normed.shape, range_attention_maps.shape))
        ...
        pred = F.interpolate(pred, scale_factor=2, mode='bilinear')  # [B, 1, 480, 640]
        pred = self.decoder_up5(x, pred)  # [B, 1, 480, 640]
        # print("pred.shape:", pred.shape)
