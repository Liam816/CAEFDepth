...
class FasterNetEdgePCGUB(nn.Module):
    def __init__(self,
                 opts,
                 in_chans=3,
                 num_classes=1000,
        ...
        self.stages = nn.Sequential(*stages_list)  # 4个stage 夹着3层Merging
    ...
    def data_normalization(self, x: Tensor):
        x_ = 255. * (x - x.min()) / (x.max() - x.min())
        return x_.to(torch.uint8)

    def data_denormalization(self, x: Tensor, vmin: torch.float32, vmax: torch.float32):
        x_ = vmin + ((vmax - vmin) * x / 255.)
        return x_.to(torch.float32)

    # # NOTE: for循环对每个特征图sharpen
    ...
    #     # temp = []
    #     # for i in range(y.size(0)):  # batch
    #     #     temp1 = VF.adjust_sharpness(y[i, 0:1, :, :], sharpen_factor)
    #     #     for j in range(y.size(1)):  # channels
    #     #         if j >= 1:
    #     #             temp11 = VF.adjust_sharpness(y[i, j:j+1, :, :], sharpen_factor)
    #     #             temp1 = torch.cat((temp1, temp11), dim=1)
    #     #             if j == y.size(1) - 1:
    #     #                 temp.append(temp1)
    #     temp = []
    #     for i in range(y.size(0)):  # batch
    #         for j in range(y.size(1)):  # channels
    #             temp.append(VF.adjust_sharpness(y[i:i+1, j:j+1, :, :], sharpen_factor))
    #     temp1 = []
    #     for i in range(y.size(0)):
    #         temp1.append(torch.cat(temp[i*y.size(1):(i+1)*y.size(1)], dim=1))
    #     y = torch.cat(temp1, dim=0)
    ...
    def decoder_upsample(self, x: Tensor, feature: Tensor, *args, **kwargs):
        ...
        y = F.interpolate(y, scale_factor=2, mode='bilinear')  # [B, 256, 30, 40]
        '''feature maps sharpen'''
        b, c, h, w = y.size()
        vmin = y.min()
        vmax = y.max()
        y = torch.reshape(y, (b*c, 1, h, w))
        y = self.data_normalization(y)  # 将特征图值域归一化到图像的像素值域区间[0, 255]
        y = VF.adjust_sharpness(y, sharpen_factor)
        y = torch.reshape(y, (b, c, h, w))
        y = self.data_denormalization(y, vmin=vmin, vmax=vmax)
        '''===================='''
        y = self.decoder_up1(guide_maps[-1], y)  # [B, 128, 30, 40]
        features_.append(y)
    ...
    #         x = self.stages[i](x)
    #         # print("x_{} shape:{}".format(i, x.shape))
    #         encoder_features.append(x)
    #
    #     x = self.btlnck_conv(x)  # [B, 256, 15, 20]
    #
    #     # NOTE: decoder upsample
    #     decoder_res = self.decoder_upsample(shortcut, x)
    #
    #     # for i in range(len(res)):
    #     #     print("decoder_{}:{}".format(i, res[i].shape))
    #
    #     return encoder_features, decoder_res
