...
from ..modules import Guided_Upsampling_Block
...
class FasterNetGUB(nn.Module):
    def __init__(self,
        ...
        logger.info("embed_dim in class FasterNetGUB:{}".format(embed_dim))
        ...
        for i_stage in range(self.num_stages):
            # print("sum(depths[:i_stage])={}".format(sum(depths[:i_stage])))
            # print("sum(depths[:i_stage + 1])={}".format(sum(depths[:i_stage + 1])))
            # temp = dpr[sum(depths[:i_stage]):sum(depths[:i_stage + 1])]
            # print("============temp dpr:\n", temp)
