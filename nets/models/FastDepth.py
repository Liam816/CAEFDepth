...
def choose_decoder(decoder):
    ...
    if decoder[:6] == 'deconv':
        ...
        assert len(decoder)==7 or (len(decoder)==9 and 'dw' in decoder)
        kernel_size = int(decoder[6])
        print('kernel_size:{} depthwise:{}'.format(kernel_size, depthwise))
...
class FastDepthMobileNet(nn.Module):
    def __init__(self, output_size=None, decoder='nnconv5dw', in_channels=3, pretrained=True):

        super(FastDepthMobileNet, self).__init__()
        self.output_size = output_size
        # mobilenet = imagenet.mobilenet.MobileNet()
        mobilenet = nets.modules.mobilenet.MobileNet()
        if pretrained:
            # pretrained_path = os.path.join('imagenet', 'results', 'imagenet.arch=mobilenet.lr=0.1.bs=256', 'model_best.pth.tar')
            # pretrained_path = os.path.join('./weights', 'FastDepth', 'imagenet.arch=mobilenet.lr=0.1.bs=256',
            #                                'model_best.pth.tar')
            pretrained_path = '/home/data/glw/hp/projects/GuidedDecoding_2023-09-02/nets/models/weights/FastDepth/' \
            ...
            missing_keys, unexpected_keys = mobilenet.load_state_dict(new_state_dict)

            if len(unexpected_keys) > 0:
                print("There exists unexpected keys in FastDepth MobileNet.")
                # print("[unexpected_keys]:", *unexpected_keys, sep="\n")
            if len(missing_keys) > 0:
                print("There exists missing keys in FastDepth MobileNet.")
                # print("[missing_keys]:", *missing_keys, sep="\n")
            if len(unexpected_keys) == 0 and len(missing_keys) == 0:
                print("FastDepth MobileNet pretrained weights loaded.")
...
class FastDepthMobileNetSkipAdd(nn.Module):
    def __init__(self, output_size, pretrained=True):

        super(FastDepthMobileNetSkipAdd, self).__init__()
        self.output_size = output_size
        # mobilenet = imagenet.mobilenet.MobileNet()
        mobilenet = nets.modules.mobilenet.MobileNet()
        if pretrained:
...
class FastDepthMobileNetSkipConcat(nn.Module):
    def __init__(self, output_size, pretrained=True):

        super(FastDepthMobileNetSkipConcat, self).__init__()
        self.output_size = output_size
        # mobilenet = imagenet.mobilenet.MobileNet()
        mobilenet = nets.modules.mobilenet.MobileNet()
        if pretrained:
